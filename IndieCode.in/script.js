...
async function loadFeaturedProducts() {
    ...
    if (!slider || slider.dataset.hydrated === 'true' || !window.supabaseClient) return;
    ...
    try {
        ...
        data.forEach((record, idx) => {
            slider.appendChild(buildFeaturedProductCard(record, idx));
        });
...
function buildFeaturedProductCard(record, idx) {
    const card = document.createElement('article');
    const delayClass = `reveal-delay-${(idx % 4) + 1}`;
    card.className = `project-card reveal-on-scroll ${delayClass}`;
    card.style.cursor = 'pointer';
    card.onclick = () => {
        window.location.href = `product.html?id=${record.slug}`;
    };

    const badgeVariant = record.badge_variant || 'other';
    let badgeClass = 'project-badge';
    if (badgeVariant === 'ready') badgeClass += ' project-badge-ready';
    else if (badgeVariant === 'custom') badgeClass += ' project-badge-custom';

    const summary = record.card_summary || record.description || 'Launch-ready build tailored for fast teams.';
    const badgeLabel = record.badge_label || (badgeVariant === 'ready' ? 'Ready Product' : badgeVariant === 'custom' ? 'Custom Build' : 'Product');
    const priceLabel = record.price_label || '';
    const desktopImages = parseProductImagesField(record.desktop_images);
    const image = record.card_image || desktopImages[0] || '';

    card.innerHTML = `
        <div class="project-image">
            ${priceLabel ? `<div style="position: absolute; top: 16px; right: 16px; z-index: 5;"><span class="project-price-tag">${priceLabel}</span></div>` : ''}
            <span class="${badgeClass}">${badgeLabel}</span>
            ${image ? `<img src="${image}" alt="${record.title || 'Product'} preview">` : '<div style="height: 180px; width: 100%; border-radius: 16px; background: rgba(248,249,250,0.05);"></div>'}
        </div>
        <div class="project-content">
            <h3>${record.title || 'Product'}</h3>
            <p>${summary}</p>
            <a href="product.html?id=${record.slug}" class="project-link">View Details <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M5 12h14M12 5l7 7-7 7" /></svg></a>
        </div>
    `;
...
function parseProductImagesField(value) {
    if (!value) return [];
